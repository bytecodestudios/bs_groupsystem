        ...
        <MotionDiv layoutId={`group-card-${group.id}`} onClick={() => onSelectGroup(group)} className="bg-secondary/50 border border-border rounded-lg p-6 cursor-pointer group hover:border-emerald-400/50 transition-colors">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">{group.isLeader ? t('ui.group_card.you_are_leader') : t('ui.group_card.you_are_member')}</p>
                </div>
                <span className="text-xs font-bold px-2 py-1 rounded-full bg-green-500/20 text-green-300">{group.status}</span>
            </div>
            <div className="mt-6">
                <div className="flex justify-between items-center mb-1"><span className="text-xs font-semibold text-muted-foreground">{t('ui.group_card.members')}</span><span className="text-xs font-semibold text-foreground">{group.members.length} / {group.maxMembers}</span></div>
            ...
            <div className="flex justify-between items-end mt-4">
                ...
                <span className="text-sm font-semibold text-foreground group-hover:text-emerald-300 transition-colors">{t('ui.group_card.view_details')}</span>
            </div>
        </MotionDiv>
    );
};
...
export const PublicGroupCard: React.FC<{ group: Group, onRequestToJoin: (groupId: string) => void, hasSentRequest: boolean, isInGroup?: boolean }> = ({ group, onRequestToJoin, hasSentRequest, isInGroup }) => {
    ...
    const buttonState = useMemo(() => {
        if (isInGroup) return { text: t('ui.group_card.already_in_group'), disabled: true, style: "bg-secondary text-muted-foreground cursor-not-allowed opacity-70" };
        if (isFull) return { text: t('ui.group_card.group_is_full'), disabled: true, style: "bg-yellow-500/20 text-yellow-300 cursor-not-allowed" };
        if (hasSentRequest) return { text: t('ui.group_card.request_sent'), disabled: true, style: "bg-blue-500/20 text-blue-300 cursor-not-allowed" };
        
        if (group.joinType === 'Closed') return { text: t('ui.group_card.closed'), disabled: true, style: "bg-secondary text-muted-foreground cursor-not-allowed opacity-70" };
        if (group.joinType === 'Invite Only') return { text: t('ui.group_card.invite_only'), disabled: true, style: "bg-secondary text-muted-foreground cursor-not-allowed opacity-70" };

        if (group.status !== 'Recruiting') return { text: t('ui.group_card.not_recruiting'), disabled: true, style: "bg-secondary text-muted-foreground cursor-not-allowed" };
        return { text: t('ui.group_card.request_to_join'), disabled: false, style: "bg-secondary hover:bg-muted text-foreground border border-border transition-colors" };
    ...
    return (
        <MotionDiv layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-secondary/50 border border-border rounded-lg p-4 flex flex-col justify-between space-y-3">
            ...
            <p className="text-sm font-semibold text-muted-foreground">{t('ui.group_card.members_format', String(group.members.length), String(group.maxMembers))}</p>
